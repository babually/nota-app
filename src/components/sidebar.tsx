...
import type { FilterOption, TaskCounts } from '@/types/tssk'
    ...
    <div className="w-64 bg-white border-r border-gray-200 flex flex-col">
      ...
      <div className="p-6 border-b border-gray-100">
        <div className="flex items-center space-x-3">
          <Avatar 
            src="/placeholder-avatar.jpg" initials="SH" 
            size='md'
          />
        ...
        <div className="space-y-1">
          {menuItems.map((item) => {
            ...
            return (
              <Button
                key={item.id}
                intent={isActive ? "secondary" : "plain"}
                className={`w-full justify-start h-11 ${
                  isActive 
                    ? "bg-primary hover:bg-green-500 text-primary-foreground shadow-sm" 
                    : isUrgent
              ...
              >
                <Icon className={`h-4 w-4 mr-3 ${isUrgent ? 'text-red-500' : ''} ${isActive ? 'text-white' : ''}`} />
                ...
                >
                  <span className={`text-xs ${isActive ? 'text-white' : ''}`}>{item.count}</span>
                </Badge>
              </Button>
            )
          })}
