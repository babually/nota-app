...
import type { FilterOption, SortOption, Task } from "@/types/tssk";
...
function Tasks() {
  ...
  useEffect(() => {
    ...
    if (overdueTasks.length > 0) {
      toast.success(
        `You have ${overdueTasks.length} task${overdueTasks.length > 1 ? "s" : ""} that need attention`,
      );
    }
  }, [tasks, toast]);
  ...
  const addTask = (newTask: Omit<Task, "id" | "createdAt">) => {
    ...
    toast.success(`Task "${task.title}" has been added to your tasks`);
  };
